 ...
 */
class UserInfoEditor extends PropertyList {
    ...
    public function Save() {
        ...
        if ($this->IsNew) {
            ...
            //insert
            $sql = $conn->CreateInsertSTMT($this->dataservice->GetDataTableName('user'));
            ...
            //update
            $sql = $conn->CreateUpdateSTMT($this->dataservice->GetDataTableName('user'));
        }
        $this->MoveFirst();
    ...
    public function SetLevel($l) {
        ...
        if (!$this->dataservice->RecordExists('level', 'levNumber', $l)) {
    ...
    public function Delete($uid = '') {
        ...
        $conn = $this->dataservice->GetDBConn();
        $sql = $conn->CreateDeleteSTMT($this->dataservice->GetDataTableName('user'));
        $sql->AddEqual('usrUID', $uid, IBC1_DATATYPE_PLAINTEXT);
